...
/*
 * Matrix multiplication (C = AB) using a row-wise decomposition approach.
 * The main complexity of any parallel application is figure out how to assign chunks to processes.
 * In the following code the work is divided depending on the number of C rows.
 * For instance, if C has 4 rows and the program is run with 2 processes, master and worker (with ranks 0 and 1),
 * master will compute rows 0 and 2, worker rows 1 and 3.
*/
